	...
	else
	{
		...
		if (camera)
		{
			...
			if (_ga_t0 && _ga_t1)
			{
				...
				intersectors->addIntersector(new osgUtil::LineSegmentIntersector(osgUtil::Intersector::PROJECTION, _ga_t0->getXnormalized(), _ga_t0->getYnormalized()));
			...
			//	camera->accept(iv);
				...
				for (osg::ref_ptr<osgUtil::Intersector>& is : intersectors->getIntersectors())
				{
					...
					if (lis && lis->containsIntersections())
					{
						...
						osg::Vec3 PT = pt * osg::Matrix::inverse(camera->getProjectionMatrix()) * osg::Matrix::inverse(osg::Matrix::translate(0, 0, -_distance)) * osg::Matrix::inverse(osg::Matrix::rotate(camera->getViewMatrix().getRotate()));
			...
			osg::Vec3d dv				  = osg::Vec3(deltaPt[0] / w * screenWidthInWorld, deltaPt[1] / h * (screenWidthInWorld * (h / w)), 0);
...
void ThreeDimManipulator::rotateTrackball(const float px0, const float py0, const float px1, const float py1, const float scale)
{
#pragma region 基类的实现
	osg::Vec3d axis;
	float	  angle;
...
float getNearDistance(osg::Camera* camera, osg::BoundingBox& bb, float dist)
{
	...
	while (true)
	{
		osg::Matrix view_m = osg::Matrixd::translate(-bb.center()) * osg::Matrixd::rotate(camera->getViewMatrix().getRotate()) * osg::Matrixd::translate(0, 0, -dist);
		...
		for (int i = 0; i <= 7; i++)
		{
			osg::Vec3 pp = bb.corner(i) * view_m * camera->getProjectionMatrix() * camera->getViewport()->computeWindowMatrix();
...
void ThreeDimManipulator::rectZoomPersp()
{
	...
	intersectors->addIntersector(new osgUtil::LineSegmentIntersector(osgUtil::Intersector::PROJECTION, (pt1.x() + pt2.x()) / 2, (pt1.y() + pt2.y()) / 2));
	intersectors->addIntersector(new osgUtil::LineSegmentIntersector(osgUtil::Intersector::PROJECTION, pt1.x(), pt1.y()));
	intersectors->addIntersector(new osgUtil::LineSegmentIntersector(osgUtil::Intersector::PROJECTION, pt1.x(), pt2.y()));
	intersectors->addIntersector(new osgUtil::LineSegmentIntersector(osgUtil::Intersector::PROJECTION, pt2.x(), pt1.y()));
	intersectors->addIntersector(new osgUtil::LineSegmentIntersector(osgUtil::Intersector::PROJECTION, pt2.x(), pt2.y()));
	...
	for (float i = kDELTA; i <= 1 - kDELTA; i += SAMPLE_DELTA)
	{
		for (float j = kDELTA; j <= 1 - kDELTA; j += SAMPLE_DELTA)
		{
			...
			intersectors->addIntersector(new osgUtil::LineSegmentIntersector(osgUtil::Intersector::PROJECTION, mid.x(), mid.y()));
		}
	}
...
void ThreeDimManipulator::toggleCameraModel()
{
	...
	if (_viewer->isPerspectiveCamera())  //--------------------------------------------------透视to正交
	{
		...
		for (int i = 0; i < vecPos.size(); i++)
		{
			...
			osg::Vec4 p2 = osg::Vec4(screenPos[i], 1) * osg::Matrix::inverse(camera->getViewport()->computeWindowMatrix());
	...
	else  //--------------------------------------------------正交to透视
	{
		...
		for (int i = 0; i < vecPos.size(); i++)
		{
			...
			osg::Vec4 p1 = osg::Vec4(vecPos[i], 1) * osg::Matrixd::translate(-_center) * osg::Matrixd::rotate(_rotation.inverse());
			osg::Vec4 p2 = osg::Vec4(screenPos[i], 1) * osg::Matrix::inverse(camera->getProjectionMatrix() * camera->getViewport()->computeWindowMatrix());
			//p1 * M = p2
...
void ThreeDimManipulator::rectZoomOrth()
{
	...
	//--------------------------------------------------更改视图矩阵
	osg::Matrix mat = osg::Matrixd::translate(-_center) * osg::Matrixd::rotate(_rotation.inverse()) * osg::Matrix::translate(0, 0, 1);
